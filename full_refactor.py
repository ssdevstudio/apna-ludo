...
old_picker = '''{reactionPickerOpen && <div className="reaction-picker" style={{position:"absolute", right:0, top:"100%", background:"white", border:"1px solid #ddd", borderRadius:"8px", padding:"4px", display:"flex", gap:"4px", zIndex:100, boxShadow:"0 2px 8px rgba(0,0,0,0.1)"}}>
              {["dY`?","dY~,","dY~","dY~","dY~r","dYZ"].map(e=><button key={e} onClick={()=>sendReaction(e)} style={{background:"none", border:"none", fontSize:"1.2rem", cursor:"pointer", padding:"4px"}}>{e}</button>)}
            </div>}'''
...
app = re.sub(r'\{reactionPickerOpen && <div className="reaction-picker".*?</div>\}', new_picker, app, flags=re.DOTALL)
